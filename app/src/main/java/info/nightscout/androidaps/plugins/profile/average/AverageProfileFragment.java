...
public class AverageProfileFragment extends SubscriberFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        try {
            ...
            diaView.setParams(AverageProfilePlugin.getPlugin().dia, 2d, 48d, 0.1d, new DecimalFormat("0.0"), false, layout.findViewById(R.id.averageprofile_save), textWatch);
            ...
            icView.setParams(AverageProfilePlugin.getPlugin().ic, 0.5d, 50d, 0.1d, new DecimalFormat("0.0"), false, layout.findViewById(R.id.averageprofile_save), textWatch);
            isfView = layout.findViewById(R.id.averageprofile_isf);
            isfView.setParams(AverageProfilePlugin.getPlugin().isf, 0.5d, 500d, 0.1d, new DecimalFormat("0.0"), false, layout.findViewById(R.id.averageprofile_save), textWatch);
            ...
            resetButton.setOnClickListener(view -> {
                ...
                mgdlView.setChecked(AverageProfilePlugin.getPlugin().mgdl);
                mmolView.setChecked(AverageProfilePlugin.getPlugin().mmol);
                diaView.setParams(AverageProfilePlugin.getPlugin().dia, 2d, 48d, 0.1d, new DecimalFormat("0.0"), false, view.findViewById(R.id.averageprofile_save), textWatch);
                icView.setParams(AverageProfilePlugin.getPlugin().ic, 0.5d, 50d, 0.1d, new DecimalFormat("0.0"), false, view.findViewById(R.id.averageprofile_save), textWatch);
                isfView.setParams(AverageProfilePlugin.getPlugin().isf, 0.5d, 500d, 0.1d, new DecimalFormat("0.0"), false, view.findViewById(R.id.averageprofile_save), textWatch);
                basalView = new TimeListEdit(getContext(), layout, R.id.averageprofile_basal, MainApp.gs(R.string.nsprofileview_basal_label) + ": " + getSumLabel(), AverageProfilePlugin.getPlugin().basal, null, pumpDescription.basalMinimumRate, 10, 0.01d, new DecimalFormat("0.00"), save);
                targetView = new TimeListEdit(getContext(), layout, R.id.averageprofile_target, MainApp.gs(R.string.nsprofileview_target_label) + ":", AverageProfilePlugin.getPlugin().targetLow, AverageProfilePlugin.getPlugin().targetHigh, 3d, 200, 0.1d, new DecimalFormat("0.0"), save);
                updateGUI();
            });
